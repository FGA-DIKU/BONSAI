...
from bonsai.modules.networks.test_transformer import (
    BasicTransformerConfig,
    BasicTransformerForMaskedLM,
    BasicTransformerForSequenceClassification,
)
...
def main(cfg: DictConfig) -> None:
    ...
    print(len(data_module.vocabulary))
    # Load your custom config
    config = BasicTransformerConfig(
        vocab_size=len(data_module.vocabulary),
        hidden_size=256,  # Adjust scaling as needed
        num_hidden_layers=4,
        num_attention_heads=4,
    )

    # For Pretraining:
    model = BasicTransformerForMaskedLM(config)
