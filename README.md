...
BONSAI turns raw EHR data into tokenized patient histories, pretrains a BERT-style encoder on them, and finetunes it to predict clinical outcomes. It is the successor of [CORE-BEHRT](https://github.com/mikkelfo/CORE-BEHRT), rebuilt on PyTorch Lightning and Hydra for large-scale, reproducible experiments.
...
- **Pretraining.** Masked language modeling with a ModernBERT-style encoder (RoPE, pre-norm, flash-attention or PyTorch SDPA backend) implemented in the repo, no HuggingFace dependency.
...
- **Finetuning and prediction.** Binary outcome prediction from a linear head on the `[CLS]` token, predictions on a held-out split, warmup learning rate schedules and resumable, versioned runs.
...
```bash
# 1. MEDS -> features + tokenized sequences, saved to data/correlated_MEDS_data
...
# 3. Build outcome labels -> data/correlated_MEDS_data/outcomes/examples/example_outcome1.parquet
python bonsai/run/create_outcome.py --config-name examples/example_outcome1 dataset=correlated_MEDS_data

# 4. Finetune the pretrained encoder on that outcome
...
**Resume training** by passing the checkpoint and the original run id. Without `run_id` a new run directory is created.
...
The pre-Lightning version is preserved at the `pre-lightning` tag.
